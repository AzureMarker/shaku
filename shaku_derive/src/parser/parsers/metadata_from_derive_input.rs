...
impl Parser<MetaData> for syn::DeriveInput {
    fn parse_into(&self) -> Result<MetaData, DIError> {
        ...
        if let Some(ref interface_attribute) = self
            .attrs
            .iter()
            .find(|a| a.path.is_ident(consts::INTERFACE_ATTR_NAME))
        {
            ...
            match interface_attribute.parse_meta().unwrap() {
                syn::Meta::Path(_) => Err(DIError::ParseError(format!(
                    "invalid attribute format > '{:?}' the name of the trait is missing",
                    interface_attribute
                ))),

                syn::Meta::NameValue(lit) => Ok(MetaData {
                    interface: Some(lit.path.get_ident().unwrap().clone()),
                }),

                syn::Meta::List(nested) => {
                    let mut traits: Vec<syn::Ident> = nested
                        .nested
                        .iter()
            ...
            Err(DIError::ParseError(format!(
                "unable to find {} > please add a '#[{}(<your trait>)]'",
